...
export class Fused extends Component {
	constructor(props) {
	    super(props);
	    this.state={
	    	dragens:[]
	...
	componentDidMount() {
		...
		if(id.indexOf(',')<0){ //单只龙
			...
			call(contractAddress, 0, 'getMBI',callArgs, (resp) =>{
				...
				that.setState({
					dragens:[processDragen(dragen,id,'fused')]
				})
			})
		}
		// else{//多只龙
		...
		// 			dragens.push(processDragen(dragen,v,'fused'))
			...
			<div id="hatched">
				<div className="container_960px">
					<div className="title" style={state.dragens.length > 1?{lineHeight:'1',paddingTop:'30px'}:{}}>
						<p style={state.dragens.length === 1?{marginBottom:'0'}:{}}>Fusion completed</p>
					</div>
